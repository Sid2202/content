...
class WP_Freeio_Job_Filter extends WP_Freeio_Abstract_Filter {
	...
	public static function get_fields() {
		return apply_filters( 'wp-freeio-default-job_listing-filter-fields', array());
	}
	...
	public static function taxonomy($query) {
		...
		if ( is_tax( 'job_listing_type' ) || is_tax( 'job_listing_tag' ) || is_tax( 'location' ) || is_tax( 'job_listing_category' ) || apply_filters( 'wp-freeio-job-query-taxonomy', false ) ) {
	...
	public static function filter_query( $query = null, $params = array() ) {
		...
		// Filter params
		$params = apply_filters( 'wp_freeio_job_filter_params', $params );
		...
		return apply_filters('wp-freeio-job_listing-filter-query', $query, $params);
	}

	public static function get_query_var_filter($query_vars, $params) {
		$ids = null;
		$query_vars = self::orderby($query_vars, $params, WP_FREEIO_JOB_LISTING_PREFIX);

		// Property title
		if ( ! empty( $params['filter-title'] ) ) {
			global $wp_freeio_job_keyword;
			$wp_freeio_job_keyword = sanitize_text_field( wp_unslash($params['filter-title']) );
	...
	public static function get_meta_filter($params) {
    	...
    	if ( isset( $params['filter-salary-from'] ) && isset( $params['filter-salary-to'] ) ) {
			...
			if ( isset($params['filter-salary-from']) && intval($params['filter-salary-from']) >= 0 && isset($params['filter-salary-to']) && intval($params['filter-salary-to']) > 0) {
				...
				$salary_from = $params['filter-salary-from'];
				$salary_to = $params['filter-salary-to'];

				$price_meta_query[] = array(
		           	'key' => WP_FREEIO_JOB_LISTING_PREFIX . 'salary',
		       	...
		       	$price_meta_query[] = array(
		           	'key' => WP_FREEIO_JOB_LISTING_PREFIX . 'max_salary',
		           	'value' => array( sanitize_text_field($salary_from), sanitize_text_field($salary_to) ),
		           	'compare'   => 'BETWEEN',
					'type'      => 'NUMERIC',
		       	);
			...
			if ( ! empty( $params['filter-'.$meta_key] ) ) {
				...
				if ( is_array($value) ) {
					...
					foreach ($value as $val) {
						...
						$multi_meta[] = array(
							'key'       => WP_FREEIO_JOB_LISTING_PREFIX . $meta_key,
							'value'     => $f_val,
							'compare'   => $compare,
						);
					}
	...
	public static function get_jobs_keyword_search( $search ) {
		global $wpdb, $wp_freeio_job_keyword;
	    ...
	    $search = '';
	   	if ( $wp_freeio_job_keyword ) {
	        $search = "($wpdb->posts.post_title LIKE '%{$wp_freeio_job_keyword}%')";
	    }
		...
		$searchable_meta_keys = array(
			WP_FREEIO_JOB_LISTING_PREFIX.'address',
		);
		...
		if ( apply_filters( 'wp_freeio_search_post_meta', true ) ) {
			...
			if ( $searchable_meta_keys ) {
				$conditions[] = "{$wpdb->posts}.ID IN ( SELECT post_id FROM {$wpdb->postmeta} WHERE meta_key IN ( '" . implode( "','", array_map( 'esc_sql', $searchable_meta_keys ) ) . "' ) AND meta_value LIKE '%" . esc_sql( $wp_freeio_job_keyword ) . "%' )";
			} else {
				// No meta keys defined, search all post meta value.
				$conditions[] = "{$wpdb->posts}.ID IN ( SELECT post_id FROM {$wpdb->postmeta} WHERE meta_value LIKE '%" . esc_sql( $wp_freeio_job_keyword ) . "%' )";
			}
		}

		// Search taxonomy.
		$conditions[] = "{$wpdb->posts}.ID IN ( SELECT object_id FROM {$wpdb->term_relationships} AS tr LEFT JOIN {$wpdb->term_taxonomy} AS tt ON tr.term_taxonomy_id = tt.term_taxonomy_id LEFT JOIN {$wpdb->terms} AS t ON tt.term_id = t.term_id WHERE t.name LIKE '%" . esc_sql( $wp_freeio_job_keyword ) . "%' )";
		
		$conditions = apply_filters( 'wp_freeio_search_conditions', $conditions, $wp_freeio_job_keyword );
		if ( empty( $conditions ) ) {
			return $search;
		}
	...
	public static function display_filter_value_simple($key, $value, $filters) {
		...
		switch ($key) {
			case 'filter-category':
				self::render_filter_tax_simple($key, $value, 'job_listing_category', esc_html__('Category', 'wp-freeio'));
				break;
			case 'filter-location':
				self::render_filter_tax_simple($key, $value, 'location', esc_html__('Location', 'wp-freeio'));
				break;
			case 'filter-type':
				self::render_filter_tax_simple($key, $value, 'job_listing_type', esc_html__('Type', 'wp-freeio'));
				break;
			case 'filter-tag':
				self::render_filter_tax_simple($key, $value, 'job_listing_tag', esc_html__('Tag', 'wp-freeio'));
				break;
			case 'filter-salary':
				...
				if ( !empty($value) ) {
					if ( is_array($value) ) {
						foreach ($value as $val) {
							...
							self::render_filter_result_item_simple( $title, esc_html__('Author', 'wp-freeio') );
						}
					} else {
						$employer_id = WP_Freeio_User::get_employer_by_user_id($value);
						if ( $employer_id ) {
			...
			default:
				...
				if (preg_match("/-to/i", $key)) {
					$prefix = esc_html__('to', 'wp-freeio');

					$label_key = str_replace('-to', '', $label_key);
				} elseif (preg_match("/-from/i", $key)) {
					$prefix = esc_html__('from', 'wp-freeio');
