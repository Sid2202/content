 ...
 */
require_once WP_FREEIO_PLUGIN_DIR . 'libraries/facebook-sdk/autoload.php';
...
class WP_Freeio_Social_Facebook {
	...
	public function __construct() {
        ...
        $this->app_id = wp_freeio_get_option( 'facebook_api_app_id' );
        $this->app_secret = wp_freeio_get_option( 'facebook_api_app_secret' );

        $this->callback_url = admin_url('admin-ajax.php?action=wp_freeio_facebook_login');
        if ( ($this->is_facebook_login_enabled() || $this->is_facebook_apply_enabled()) && !is_user_logged_in() ) {
            ...
            // Ajax endpoints.
            add_action( 'wpfi_ajax_wp_freeio_facebook_login', array( $this, 'process_facebook_login' ) );
            
            // compatible handlers.
            add_action( 'wp_ajax_wp_freeio_facebook_login', array( $this, 'process_facebook_login' ) );
            add_action( 'wp_ajax_nopriv_wp_freeio_facebook_login', array( $this, 'process_facebook_login' ) );
            
            add_action( 'wp_freeio_after_facebook_login', array( $this, 'process_apply_job'), 10, 1 );
    ...
    public function is_facebook_login_enabled() {
        if ( wp_freeio_get_option('enable_facebook_login') && ! empty( $this->app_id ) && ! empty( $this->app_secret ) ) {
            return true;
        }
    ...
    public function is_facebook_apply_enabled() {
        if ( wp_freeio_get_option('enable_facebook_apply') && ! empty( $this->app_id ) && ! empty( $this->app_secret ) ) {
            return true;
        }
    ...
    private function get_token($fb) {
        ...
        $url = admin_url('admin-ajax.php?action=wp_freeio_facebook_login');
        try {
            $accessToken = $helper->getAccessToken($url);
        } catch (FacebookResponseException $e) {
            $msg = sprintf(__('Graph returned an error: %s', 'wp-freeio'), $e->getMessage());
        } catch (FacebookSDKException $e) {
            $msg = sprintf(__('Facebook SDK returned an error: %s', 'wp-freeio'), $e->getMessage());
        }

        // Check Error
        if (!isset($accessToken)) {
            // Errors Message
            set_transient('wp_freeio_facebook_message', $msg, 60 * 60 * 24 * 30);
    ...
    private function login_user() {
        ...
        if ( $user_login_auth == 'pending' ) {
            ...
            do_action('wp_freeio_after_facebook_login', $wp_users[0]);
        }
    ...
    private function create_user() {
        ...
        if ( !empty($wp_user->ID) ) {
            update_user_meta($wp_user->ID, 'wp_freeio_facebook_id', $facebook_user_data['id']);
            $this->login_user();
        }
        ...
        $userdata = apply_filters('wp-freeio-facebook-login-userdata', $userdata, $facebook_user_data);

        $_POST['role'] = 'wp_freeio_freelancer';
        $_POST['action'] = 'wp_freeio_ajax_register';
        
        global $wp_freeio_socials_register;
        $wp_freeio_socials_register = $facebook_user_data['id'];
        ...
        if ( !is_wp_error( $user_id ) ) {
            ...
            update_user_meta($user_id, 'wp_freeio_facebook_id', $facebook_user_data['id']);
            
            
        	do_action('wp_freeio_after_facebook_login', $user_id);

			wp_set_auth_cookie($user_id);
            
        } else {
	        set_transient('wp_freeio_facebook_message', $user_id->get_error_message(), 60 * 60 * 24 * 30);
    ...
    public function process_apply_job($user_id) {
        if ( isset( $_COOKIE['wp_freeio_facebook_job_id'] ) && $_COOKIE['wp_freeio_facebook_job_id'] > 0 ) {
            $job_id = $_COOKIE['wp_freeio_facebook_job_id'];
            $job = get_post($job_id);
            ...
            unset($_COOKIE['wp_freeio_facebook_job_id']);
            setcookie('wp_freeio_facebook_job_id', null, -1, '/');
        }
    }

    public function display_message() {
        if ( get_transient('wp_freeio_facebook_message') ) {
            $message = get_transient('wp_freeio_facebook_message');
            echo '<div class="alert alert-danger facebook-message">' . $message . '</div>';
            delete_transient('wp_freeio_facebook_message');
        }
    }
    ...
    public function display_login_btn() {
    	...
    	<div class="facebook-login-btn-wrapper">
    		<a class="facebook-login-btn" href="<?php echo esc_url($this->get_login_url()); ?>"><i class="fab fa-facebook-f"></i> <?php esc_html_e('Facebook', 'wp-freeio'); ?></a>
    	</div>
    	<?php
    	$output = ob_get_clean();
    	echo apply_filters('wp-freeio-facebook-login-btn', $output, $this);
    }
	...
	public function display_apply_btn($job) {
    	...
    	<div class="facebook-apply-btn-wrapper">
    		<a class="facebook-apply-btn" href="<?php echo esc_url($this->get_login_url()); ?>" data-job_id="<?php echo esc_attr($job->ID); ?>"><i class="fab fa-facebook-f"></i> <?php esc_html_e('Facebook', 'wp-freeio'); ?></a>
    	</div>
    	<?php
    	$output = ob_get_clean();
    	echo apply_filters('wp-freeio-facebook-apply-btn', $output, $this, $job);
    }

}

function wp_freeio_social_facebook() {
    WP_Freeio_Social_Facebook::get_instance();
}
add_action( 'init', 'wp_freeio_social_facebook' );