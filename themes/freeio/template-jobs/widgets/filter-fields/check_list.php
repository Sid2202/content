...
if ( !empty($options) ) {
    ...
    foreach ($options as $option) {
        ...
        ?>
        <li class="list-item <?php echo esc_attr($i > 4 ? 'more-fields' : ''); ?>"><input id="<?php echo esc_attr($name.'-'.sanitize_title($option['text'])); ?>" type="checkbox" name="<?php echo esc_attr($name); ?>[]" value="<?php echo esc_attr($option['value']); ?>" <?php echo trim($checked); ?>><label for="<?php echo esc_attr($name.'-'.sanitize_title($option['text'])); ?>"><?php echo trim($option['text']); ?></label>
        </li>
        <?php
        $i++;
    }
